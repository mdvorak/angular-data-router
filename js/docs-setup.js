NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$broadcast $data $dataresponse $dataurl $error $http $on $routechange $routeupdate $scope actual api application args argument arguments attributes automatically behaves behavior call callback calling case change changes code config configuration controller copy corresponds created currently data dataas depends deregister deregistered destroyed directly don error errors event events events_ example fails false fired fires forcereload full function handler handles headers http including instance js lifecycle listen listener load loaded locals main matched media mediatype method methods_reload modified modify module normalized note null number object occur parameter pass performed pretty propagated property provided recompiled recreate recreated refresh refreshed registers reload reloading reloads remains remover request required resource response route router routeupdatelistener samplectrl scope sense service set special status statustext succeeds text true type undefined update updated updates url variable view"
    },
    {
      "section": "api",
      "id": "locals.$dataType",
    ...
    },
    {
      ...
      "keywords": "$dataapiprovider $datarouterloaderprovider $datarouterprovider $datarouterregistryprovider absolute alias allows angular api apiprefix application base changed config configrouter configuration configures content controller currently data dataas default details disable disables displayed documentation enabled enables error event example examplectrl fails false force forces global handling href html includes intended javascript key loaded location mapping match mdvorakdataapi mdvorakdatarouter mediatype merged method methods_error methods_global methods_prefix methods_when module normalized object parameters parts path phase place precedence prefix prefixes provider redirect redirected redirectto reference relative resolvables resolve resource returns route router routes service set sets simple supported supports template templateurl tests type unit url view wildcards"
    },
    {
      "section": "api",
